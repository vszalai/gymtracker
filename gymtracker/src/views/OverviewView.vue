<template>
  <div class="bg-gradient-to-b from-gray to-darkgray min-h-[calc(100vh-4rem)] text-white">
    <div class="max-w-lg mx-auto bg-gray p-6 rounded-lg shadow-lg mb-10">
      ...
      <form @submit.prevent="setReminder" class="space-y-4 mt-6">
        <h2 class="font-black text-2xl mb-4 border-b border-white pb-2">Set Reminder</h2>
        <div class="flex flex-col space-y-1">
          <label for="reminder-message" class="text-sm font-semibold">Message:</label>
          <input
            id="reminder-message"
        ...
        <div class="flex flex-col space-y-1">
          <label for="reminder-delay" class="text-sm font-semibold">Delay (minutes):</label>
          <input
            id="reminder-delay"
...
export default {
  ...
  methods: {
    ...
    setReminder() {
      const delayMs = Number(this.reminderDelay) * 60 * 1000
      if (!this.reminderMessage || !delayMs || delayMs <= 0) {
        this.reminderNotification = 'Please enter a message and a valid delay.'
        return
      }
      ...
      this.reminderNotification = `Reminder set for ${this.reminderDelay} minute(s) from now.`
